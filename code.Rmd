...
ggplot(data = andamenti_compless_carni)+
  ...
  geom_point(aes(x = year, y = values, group = `Item Code`, colour = `Item`), size = 1.5)
...
rapp_cons_pop<- merge(crescita_pop, andamenti_compless_carni, by = "year")%>%
  mutate(rapp = values/population)

ggplot(data = rapp_cons_pop)+
 # geom_line(aes(x = year, y = values, group = `Item Code`,  colour = `Item`), size = 1)+
  geom_line(aes(x = year, y = rapp,  group = `Item Code`,  colour = `Item`), size = 1)
